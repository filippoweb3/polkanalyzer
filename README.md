...
This will install the Polkanalyzer package within your R environment so that the app can use it later. Note that if it is the first time using R, the package installation will trigger the installation of all dependencies (i.e., other packages necessary to run the Polkanalyzer functions).
...
- Sync Runs: Number of [synchronization runs](#how-selection-works). Increasing the number of runs will lead to higher loading time as the app has to perform multiple synchronizations.
